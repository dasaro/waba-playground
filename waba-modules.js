...
// Last updated: 2025-12-31T20:09:01.311Z
...
% Use ALL potential attacks (before discarding) to compute grounded extension
att(A,B) :- attacks_with_weight(A,B,_).
...
%%
%% Ideal = maximal admissible extension contained in all preferred extensions
%%
%% Approximation: We compute maximal admissible extensions that are "skeptical"
%% (don't conflict with potentially credulous arguments)
%%
%% This is implemented as: admissible extensions with no strictly larger
%% admissible subset within the candidate space
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% Arguments are assumptions
arg(X) :- assumption(X).

%% -------------------------------------------------------------------------
%% Attack relation: use ALL potential attacks (before discarding)
%% -------------------------------------------------------------------------
att(A,B) :- attacks_with_weight(A,B,_).

%% -------------------------------------------------------------------------
%% Primary candidate extension (in/out)
%% -------------------------------------------------------------------------
{ in(X) } :- arg(X).
...
%% -------------------------------------------------------------------------
%% Witness extension in2/out2 (another ADMISSIBLE extension)
%% -------------------------------------------------------------------------
{ in2(X) } :- arg(X).
out2(X) :- arg(X), not in2(X).

%% Conflict-free (witness)
:- in2(X), in2(Y), att(X,Y).
...
defended2(X) :- arg(X), not undefended2(X).

%% Admissible (witness)
:- in2(X), not defended2(X).

%% -------------------------------------------------------------------------
%% Ideal approximation: find admissible extensions with some maximality
%% (This is a simplification - true ideal requires intersection of preferred)
%% -------------------------------------------------------------------------

%% For now, just ensure it's admissible and use projection
%% The heuristic flags will guide toward reasonable ideal candidates

#project in/1.
#show in/1.
`,
        naive: `%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% NAIVE SEMANTICS (SATURATION, NO WEAK CONSTRAINTS)
%%
%% Naive = maximal (⊇) conflict-free extensions
%%
%% Uses saturation: rejects extensions where a witness has strictly larger size
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% -------------------------------------------------------------------------
%% Attack relation: use ALL potential attacks (before discarding)
%% This ensures the maximality check is consistent across models
%% -------------------------------------------------------------------------
att(A,B) :- attacks_with_weight(A,B,_).
...
%% -------------------------------------------------------------------------
%% Witness extension in2/out2 (another CONFLICT-FREE extension)
%% -------------------------------------------------------------------------
{ in2(X) } :- arg(X).
out2(X) :- arg(X), not in2(X).
...
%% -------------------------------------------------------------------------
%% "Better witness" = witness has STRICTLY more assumptions than current
%%
%% Superset check:
%%   bad_witness if witness misses something in current extension.
%% Strictness:
%%   witness adds at least one assumption not in current extension.
%% -------------------------------------------------------------------------
...
%% Naive condition: forbid existence of a better witness
:- better_witness.
...
`,
        preferred: `%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% PREFERRED SEMANTICS (SATURATION, NO WEAK CONSTRAINTS)
%%
...
%% Preferred condition: forbid existence of a better witness
:- better_witness.
...
%% -------------------------------------------------------------------------
%% Attack relation: use ALL potential attacks (before discarding)
%% This ensures the range computation is consistent across models
%% -------------------------------------------------------------------------
att(A,B) :- attacks_with_weight(A,B,_).
...
`,
        staged: `%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% STAGED SEMANTICS (SATURATION, NO WEAK CONSTRAINTS)
%%
%% Staged = conflict-free extensions with maximal range
%% Range = assumptions in extension OR defeated by extension
%%
%% Uses saturation: rejects extensions where a witness has strictly larger range
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% Defeated: attacked and no defense
defeated(X) :- attacked(X), not in(X).

%% Range of the extension (for staged: in OR defeated)
range(X) :- in(X).
range(X) :- defeated(X).
...
%% Defeated by witness
defeated2(X) :- attacked2(X), not in2(X).

%% Range (witness)
range2(X) :- in2(X).
range2(X) :- defeated2(X).
...
%% Staged condition: forbid existence of a better witness
:- better_witness.

#project in/1.
#show in/1.
`,
        'heuristic-naive': `%% Naive Semantics (EXPERIMENTAL)
%% Usage: clingo -n 0 --heuristic=Domain --enum-mode=domRec <files> semantics/heuristic/naive.lp
...
%% Maximality heuristic: prefer models with fewer missing assumptions
#heuristic miss(X) : assumption(X). [1,false]
`,
        'heuristic-preferred': `%% Preferred Semantics (EXPERIMENTAL)
%% Usage: clingo -n 0 --heuristic=Domain --enum-mode=domRec <files> semantics/heuristic/preferred.lp
...
#heuristic miss(X) : assumption(X). [1,false]
`,
        'heuristic-semi-stable': `%% Semi-Stable Semantics (EXPERIMENTAL)
%% Usage: clingo -n 0 --heuristic=Domain --enum-mode=domRec <files> semantics/heuristic/semi-stable.lp
...
`,
        'heuristic-staged': `%% Staged Semantics (EXPERIMENTAL)
%% Usage: clingo -n 0 --heuristic=Domain --enum-mode=domRec <files> semantics/staged.lp
...
#heuristic not_in_range(X). [1,false]
`,
        'heuristic-grounded': `%% Grounded Semantics (EXPERIMENTAL)
%% Usage: clingo -n 0 --heuristic=Domain --enum-mode=domRec <files> semantics/heuristic/grounded.lp
...
#heuristic in(X) : assumption(X). [1,false]`,
        'optN-preferred': `%% Preferred Semantics (Optimization-based - optN mode)
%%
%% Definition: Maximal (⊆) complete extensions
%%
%% Approach: Maximize extension size (minimize complement of in/1)
%% Use Clingo's optN mode to find all optimal (⊆-maximal) solutions
%%
%% Usage:
%%   clingo 0 core/base.lp semiring/godel.lp constraint/ub_max.lp \\
%%          semantics/complete.lp semantics/optimization/preferred.lp \\
%%          <framework>.lp \\
...
%% ========================================
%% MAXIMIZE EXTENSION SIZE
%% ========================================

%% Minimize the number of assumptions NOT in the extension
out(X) :- assumption(X), not in(X).
#minimize { 1@1,X : out(X) }.
...
#show in/1.
`,
        'optN-semi-stable': `%% Semi-Stable Semantics (Optimization-based - optN mode)
%%
%% Definition: Admissible extensions with maximal range
%% Range: range(S) = S ∪ S⁺ (assumptions in or defeated by extension)
%%
%% Approach: Minimize complement of range (equivalently: maximize range)
%% Use Clingo's optN mode to find all optimal (range-maximal) solutions
%%
%% Usage:
%%   clingo 0 core/base.lp semiring/godel.lp constraint/ub_max.lp \\
%%          semantics/admissible.lp semantics/optimization/semi-stable.lp \\
%%          <framework>.lp \\
%%          --opt-mode=optN --quiet=1 --project
%%
%% Flags:
...
%% ========================================
%% MINIMIZE COMPLEMENT OF RANGE
%% ========================================

miss(X) :- assumption(X), not range(X).
#minimize { 1@1,X : miss(X) }.
...
%% Optional: show range for debugging
%% #show range/1.
`,
        'optN-staged': `%% Staged Semantics (Optimization-based - optN mode)
%%
...
%% Usage:
%%   clingo 0 core/base.lp semiring/godel.lp constraint/ub_max.lp \\
%%          semantics/cf.lp semantics/optimization/staged.lp \\
%%          <framework>.lp \\
%%          --opt-mode=optN --quiet=1 --project
%%
%% Flags:
...
%% #show range/1.
`,
        'optN-ideal': `%% Ideal Semantics (Optimization-based - optN mode, requires common_pref/1)
%%
...
#show in/1.
`,
        'optN-eager': `%% Eager Semantics (Optimization-based - optN mode, requires common_complete/1)
%%
%% Definition: Maximal admissible subset of ∩Complete
%%
%% Two-step process:
%%   Step 1: Compute complete extensions with complete.lp + preferred.lp (optN)
%%          Use --enum-mode=cautious OR manually compute intersection
%%   Step 2: Create common_complete/1 facts from ∩Complete
%%   Step 3: Run this file with common_complete/1 to find eager extension
%%
%% NOTE: Eager is unique (always exactly one eager extension)
%% NOTE: grounded ⊆ ideal ⊆ eager ⊆ complete

%% ========================================
%% RESTRICT EXTENSION TO COMMON_COMPLETE
%% ========================================

%% Only assumptions in common_complete can be in the extension
:- in(X), not common_complete(X).
out(X) :- assumption(X), not common_complete(X).

%% Admissibility is enforced by loading admissible.lp

%% ========================================
%% MAXIMIZE INCLUSION INSIDE COMMON_COMPLETE
%% ========================================

%% Minimize what we leave out from common_complete
miss_common(X) :- common_complete(X), not in(X).
#minimize { 1@1,X : miss_common(X) }.
...
:- C = #max{ W : discarded_attack(_,_,W) }, C <= B, budget(B), B != #inf, C != #inf.
...
%% Using <= ensures strict boundary (beta=N requires cost > N)
:- C = #sum{ W,X,Y : discarded_attack(X,Y,W) }, C <= B, budget(B), B != #inf.
...
:- C = #count{ X,Y : discarded_attack(X,Y,_) }, C <= B, budget(B), B != #inf.
...
`,
        projection: `%% Output filtering for projection mode: show only in/1 and out/1
#show in/1.
#show out/1.
`
    }
