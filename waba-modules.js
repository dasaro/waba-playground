...
// Last updated: 2026-01-04T02:35:34.473Z
...
defeated(X) :- attacks_successfully(_,X).
...
attacks_successfully(X,Y) :- attacks_successfully_with_weight(X,Y,_).
...
undec(X) :- assumtpion(X), not in(X), not out(X).
...
`,
        cf: `%% Assumption choice: each assumption is either in or out
in(X) :- assumption(X), not out(X).
out(X) :- assumption(X), not in(X).
undec(X) :- assumtpion(X), not in(X), not out(X).
...
:- in(X), not_defended(X).
`,
        complete: `% Module not found: complete.lp`,
        grounded: `% Module not found: grounded.lp`,
        ideal: `% Module not found: ideal.lp`,
        naive: `% Module not found: naive.lp`,
        preferred: `% Module not found: preferred.lp`,
        'semi-stable': `% Module not found: semi-stable.lp`,
        staged: `% Module not found: staged.lp`,
