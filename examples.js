...
//  (4) REAL CORPUS DATA: nothing invented -- the framework, the attack structure and every weight
//      come from a published annotated corpus, so the example shows what the encoding looks like
//      when a domain supplies its own argumentative structure rather than an author designing one:
//        persuade_essay        : one PERSUADE 2.0 student essay -- Position/Claim/Evidence support,
//                                Counterclaim attacks, Rebuttal reinstates; weights are the corpus's
//                                double-blind expert effectiveness ratings (Arctic, falls / @667)
//
//  RETIRED (2026-08-01): out_of_africa, lipid_hypothesis and deorbit_plan_risk were structurally
//  identical to kpg_impact_vs_deccan / higgs_boson_discovery / detective_locked_house -- same
//  algebra, same "two theories, three evidence atoms, one residual objection" shape, differing
//  only in domain. Six near-duplicates made the list LOOK diverse while demonstrating one
//  mechanism repeatedly; one exemplar per (algebra, mechanism) pair is kept.
...
export const examples = {
    ...
    kpg_impact_vs_deccan: {
        ...
        preset: {
            semiring: 'arctic',
            defaultPolicy: 'neutral', monoid: 'sum', optimization: 'minimize',
    ...
    higgs_boson_discovery: {
        ...
        preset: {
            semiring: 'tropical',
            defaultPolicy: 'aba', monoid: 'sum', optimization: 'minimize',
            budgetMode: 'ub', budgetIntent: 'bounded', semantics: 'stable',
    ...
    persuade_essay: {
        label: 'Student essay: the objection the writer forgot',
        description: "Real corpus data: one annotated student essay from PERSUADE 2.0 (The Learning Agency Lab, CC BY 4.0), mapped to wABA with NOTHING invented -- every element and every weight is the corpus's own double-blind expert rating (Ineffective 333 / Adequate 667 / Effective 1000). Claims and Evidence support their parent, Counterclaims attack the Position, and a Rebuttal attacks a Counterclaim: reinstatement is native to the annotation, not modelled in. The writer answered one objection (cc_risk, killed by reb_worth) and FORGOT the other (cc_scared), so classically the position FALLS -- one unanswered objection fells a case backed by an Effective claim with evidence. beta=667 buys it back for exactly the price of that Adequate objection, and the discard receipt names it. This is also where the algebras visibly disagree about 'case strength': arctic sums the support tree to 3334, Godel takes the weakest link (333), Lukasiewicz erodes it to 0. Press Credibility to grade every element across the standpoint lattice -- the forgotten objection scores higher than the answered one.",
        section: 'curated',
        source: 'PERSUADE 2.0 corpus (Crossley et al. 2024), essay AAAOPP13416000050150, CC BY 4.0',
        code: `%% PERSUADE 2.0 essay AAAOPP13416000050150 -- "A Cowboy Who Rode the Waves" (grade 2/6)
%% Elements and expert effectiveness ratings are VERBATIM corpus annotation:
%%   Ineffective 333 / Adequate 667 / Effective 1000
%% e0 Position     Adequate    root
%% e1 Claim        Effective   supports e0
%% e2 Evidence     Adequate    supports e1
%% e3 Claim        Adequate    supports e0
%% e4 Counterclaim Adequate    attacks e0
%% e5 Counterclaim Adequate    attacks e0
%% e6 Rebuttal     Adequate    attacks e5
%% e7 Claim        Ineffective supports e0

assumption(position). contrary(position,def_position).
weight(position,667).
assumption(claim_sea). contrary(claim_sea,def_claim_sea).
weight(claim_sea,1000).
assumption(evid_sea). contrary(evid_sea,def_evid_sea).
weight(evid_sea,667).
assumption(claim_ww2). contrary(claim_ww2,def_claim_ww2).
weight(claim_ww2,667).
assumption(cc_scared). contrary(cc_scared,def_cc_scared).
weight(cc_scared,667).
assumption(cc_risk). contrary(cc_risk,def_cc_risk).
weight(cc_risk,667).
assumption(reb_worth). contrary(reb_worth,def_reb_worth).
weight(reb_worth,667).
assumption(claim_money). contrary(claim_money,def_claim_money).
weight(claim_money,333).
head(s_position,ok_position). body(s_position,position). body(s_position,ok_claim_sea). body(s_position,ok_claim_ww2). body(s_position,ok_claim_money).
head(s_claim_sea,ok_claim_sea). body(s_claim_sea,claim_sea). body(s_claim_sea,ok_evid_sea).
head(s_evid_sea,ok_evid_sea). body(s_evid_sea,evid_sea).
head(s_claim_ww2,ok_claim_ww2). body(s_claim_ww2,claim_ww2).
head(s_cc_scared,ok_cc_scared). body(s_cc_scared,cc_scared).
head(a_cc_scared,def_position). body(a_cc_scared,ok_cc_scared).
head(s_cc_risk,ok_cc_risk). body(s_cc_risk,cc_risk).
head(a_cc_risk,def_position). body(a_cc_risk,ok_cc_risk).
head(s_reb_worth,ok_reb_worth). body(s_reb_worth,reb_worth).
head(a_reb_worth,def_cc_risk). body(a_reb_worth,ok_reb_worth).
head(s_claim_money,ok_claim_money). body(s_claim_money,claim_money).`,
        preset: {
            semiring: 'arctic', defaultPolicy: 'neutral', monoid: 'max',
            optimization: 'minimize', budgetMode: 'ub', budgetIntent: 'bounded',
            semantics: 'stable', optMode: 'ignore', beta: 667,
            graphMode: 'assumption-branching'
        }
    }
};
